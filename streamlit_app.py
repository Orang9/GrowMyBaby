...
from utils.db import add_anak, delete_anak, get_anak, get_user, add_user, fetch_users, get_session_ids_for_user, save_session_to_db, load_session_from_db, get_session_name, delete_session_from_db, update_anak
...
if st.session_state.logged_in == True:
    ...
    with sidebar_placeholder.container():
        ...
        selected_page = st.sidebar.radio("Navigation", ["Home","You'r Baby" , "ChatBot", "Logout"])
    ...
    if selected_page == "Home":
        ...
        if st.button("Post to MongoDB", key='post_to_mongodb_button'):
            ...
            add_anak(nama_anak, usia_anak, jenis_kelamin, berat_badan_anak, tinggi_badan_anak, waktu_pengambilan_data)
        ...
        BB = st.number_input("Berat Badan", key="BB_input")
        TB = st.number_input("Tinggi Badan", key="TB_input")
        waktu_pengambilan_data = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        user_id = st.session_state.username

        if st.button("Tambahkan", key="tambahkan_button"):
            add_anak(nama, umur, jenis_kelamin, BB, TB, waktu_pengambilan_data, user_id)
            st.success("Data anak berhasil ditambahkan!")
            st.experimental_rerun()

        if st.button("Update", key="update_button"):
            update_anak(nama, umur, jenis_kelamin, BB, TB, waktu_pengambilan_data, user_id)
